...
export const addDrugApi = async (drugData: { ten: string; dang: string; giaca: number }) => {
  const endpoint = '/thuoc/add'
  const res = await apiClient.post(endpoint, drugData)
  return res.data
}

export const updateDrugApi = async (drugData: { maso: string; ten: string; dang: string; giaca: number }) => {
